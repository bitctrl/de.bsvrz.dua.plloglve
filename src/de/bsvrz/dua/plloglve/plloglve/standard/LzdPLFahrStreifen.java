	...
	protected Data plausibilisiere(final ResultData resultat){
		...
		if(resultat.getData() != null){
			try{
				 ...
				 */
				long qKfz = copy.getItem("qKfz").getUnscaledValue("Wert").longValue(); //$NON-NLS-1$ //$NON-NLS-2$
				long qLkw = copy.getItem("qLkw").getUnscaledValue("Wert").longValue(); //$NON-NLS-1$ //$NON-NLS-2$
				...
				if(qKfz >= 0 && qLkw >= 0){
					if(qKfz < qLkw){
						copy.getItem("qKfz").getUnscaledValue("Wert").set(DUAKonstanten.FEHLERHAFT); //$NON-NLS-1$ //$NON-NLS-2$
						copy.getItem("qKfz").getItem("Status").getItem("MessWertErsetzung").   //$NON-NLS-1$//$NON-NLS-2$//$NON-NLS-3$
							getUnscaledValue("Implausibel").set(DUAKonstanten.JA); //$NON-NLS-1$
						copy.getItem("qKfz").getItem("Güte").getUnscaledValue("Index").set(0); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
						
						copy.getItem("qLkw").getUnscaledValue("Wert").set(DUAKonstanten.FEHLERHAFT); //$NON-NLS-1$ //$NON-NLS-2$
						copy.getItem("qLkw").getItem("Status").getItem("MessWertErsetzung").   //$NON-NLS-1$//$NON-NLS-2$//$NON-NLS-3$
							getUnscaledValue("Implausibel").set(DUAKonstanten.JA); //$NON-NLS-1$
						copy.getItem("qLkw").getItem("Güte").getUnscaledValue("Index").set(0); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
					}
				}				
				/**
				 * Aenderung Ende:
