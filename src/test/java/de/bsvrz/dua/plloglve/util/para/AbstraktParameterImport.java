 ...
 */
public abstract class AbstraktParameterImport extends CSVImporter implements ClientSenderInterface {
	...
	public AbstraktParameterImport(final ClientDavInterface dav, final SystemObject objekt, final String csvQuelle)
		...
		ddLogisch = new DataDescription(this.getParameterAtg(),
				AbstraktParameterImport.sDav.getDataModel().getAspect(DaVKonstanten.ASP_PARAMETER_VORGABE));

		ddDiff = new DataDescription(diffFs,
				AbstraktParameterImport.sDav.getDataModel().getAspect(DaVKonstanten.ASP_PARAMETER_VORGABE));
		...
		if (!AbstraktParameterImport.sDav.getDataModel().getAttributeGroup(AtgVerkehrsDatenLZIPlPruefLogisch.getPid())
			...
			AbstraktParameterImport.sDav.subscribeSender(this, objekt, ddLogisch, SenderRole.sender());
			AbstraktParameterImport.sDav.subscribeSender(this, objekt, ddDiff, SenderRole.sender());
			AbstraktParameterImport.sDav.subscribeSender(this, objekt, ddAusfall, SenderRole.sender());
			AbstraktParameterImport.sDav.subscribeSender(this, objekt, ddVertrauensBereich, SenderRole.sender());
			...
			AbstraktParameterImport.sDav.unsubscribeSender(this, objekt, ddVertrauensBereich);
		}
	}
	...
	public final void importiereParameter(final int index) throws Exception {
		...
		final Data parameter = AbstraktParameterImport.sDav.createData(this.getParameterAtg());
	 ...
	 *             wird weitergereicht
	 */
	private void deaktiviereParaStandard(final OptionenPlausibilitaetsPruefungLogischVerkehr optionen)
			throws Exception {
		final Data parameter = AbstraktParameterImport.sDav.createData(this.getParameterAtg());
	...
	public final void importParaAusfall() throws Exception {
		...
		final Data parameter = AbstraktParameterImport.sDav.createData(ausfallHFs);
		DUAUtensilien.getAttributDatum("maxAusfallProTag", parameter).asUnscaledValue().set(3); //$NON-NLS-1$

		final ResultData resultat = new ResultData(this.objekt,
				new DataDescription(ausfallHFs,
						AbstraktParameterImport.sDav.getDataModel().getAspect(DaVKonstanten.ASP_PARAMETER_VORGABE)),
				System.currentTimeMillis(), parameter);
		AbstraktParameterImport.sDav.sendData(resultat);
	...
	public final void importParaVertrauensbereich() throws Exception {
		...
		final Data parameter = AbstraktParameterImport.sDav.createData(vertrauensbereichFs);
	 ...
	 */
	protected abstract String getAttributPfadVon(final String attributInCSVDatei, final int index);
