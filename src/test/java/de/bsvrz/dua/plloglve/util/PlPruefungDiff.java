...
public class PlPruefungDiff implements ClientSenderInterface, PlPruefungInterface {
	...
	public PlPruefungDiff(final ClientDavInterface dav) {
		...
		PlPruefungDiff.ddKzdSend = new DataDescription(this.dav.getDataModel().getAttributeGroup(DUAKonstanten.ATG_KZD),
		...
		try {
			...
			PlPruefungDiff.LOGGER.error("Kann Test nicht konfigurieren: " + e); //$NON-NLS-1$
		}
	}
	...
	public void pruefe() throws Exception {
		 ...
		 * Sender anmelden
		 */
		this.dav.subscribeSender(this, PlPruefungDiff.fahrStreifen, PlPruefungDiff.ddKzdSend, SenderRole.source());
		 ...
		 */
		final FilterMeldung meldFilter = new FilterMeldung(this, dav, "konstant", 105, meldungHyst); //$NON-NLS-1$
		...
		for (int i = 0; i < 5; i++) {
			while ((zeileFSDiff = fsImpFSDiff
					.getNaechstenDatensatz(PlPruefungDiff.ddKzdSend.getAttributeGroup())) != null) {
				...
				final ResultData resultat1 = new ResultData(PlPruefungDiff.fahrStreifen, PlPruefungDiff.ddKzdSend,
						aktZeit, zeileFSDiff);
		...
		PlPruefungDiff.LOGGER.info("Warte 30 Sekunden auf Meldungsfilter"); //$NON-NLS-1$
		try {
			Thread.sleep(5000L);
		...
		if (!meldFilter.wurdeAnzahlEingehalten()) {
			if (useAssert) {
				...
				PlPruefungDiff.LOGGER.warning(warnung);
			}
		} else {
		 ...
		 * Sender abmelden
		 */
		this.dav.unsubscribeSender(this, PlPruefungDiff.fahrStreifen, PlPruefungDiff.ddKzdSend);
	}
