...
public class LzdPLFahrStreifen extends KzdPLFahrStreifen {
	...
	protected Data plausibilisiere(final ResultData resultat) {
		...
		if (resultat.getData() != null) {
			...
			final long qLkw = copy.getItem("qLkw").getUnscaledValue("Wert").longValue(); //$NON-NLS-1$ //$NON-NLS-2$
	...
	protected void ueberpruefe(final Data data, final ResultData resultat) {
		if (parameterAtgLog != null) {
			synchronized (parameterAtgLog) {
				 ...
				 */
				untersucheAufMaxVerletzung(data, resultat, "qKfz", parameterAtgLog.getQKfzBereichMax());

				/**
				 * Regel Nr.14 (aus SE-02.00.00.00.00-AFo-4.0, S.95)
				 */
				untersucheAufMaxVerletzung(data, resultat, "qLkw", parameterAtgLog.getQLkwBereichMax());
			}
		}
	}
				...
				if ((wert >= 0) && (max >= 0)) {
					...
					if (maxVerletzt) {
						if (optionen.equals(OptionenPlausibilitaetsPruefungLogischVerkehr.SETZE_MAX)
							...
							LzdPLFahrStreifen.LOGGER.fine(VerwaltungPlPruefungLogischLVE.getPlLogIdent(resultat)
							...
							try {
								...
								LzdPLFahrStreifen.LOGGER.error(
		...
		if (!qPkwEmpfangenIstBereitsBerechnet && !vKfzEmpfangenIstBereitsBerechnet) {
			...
			if ((qKfz >= 0) && !qKfzImplausibel) {
				if ((qLkw >= 0) && !qLkwImplausibel) {
					if (qLkw > qKfz) {
						LzdPLFahrStreifen.LOGGER.fine(VerwaltungPlPruefungLogischLVE.getPlLogIdent(originalDatum)
						...
						try {
							...
							e.printStackTrace();
							LzdPLFahrStreifen.LOGGER.error("Berechnung der Guete von qPkw fehlgeschlagen", e); //$NON-NLS-1$
						}
					}
				} else {
					qPkw = qKfz;
			...
			if ((qKfz > 0) && (qPkw >= 0) && (vPkw >= 0) && (qLkw >= 0) && (vLkw >= 0)) {
				...
				try {
					...
					e.printStackTrace();
					LzdPLFahrStreifen.LOGGER.error("Berechnung der Guete von vKfz fehlgeschlagen", e); //$NON-NLS-1$
				}
			} else {
				LzdPLFahrStreifen.LOGGER.fine(VerwaltungPlPruefungLogischLVE.getPlLogIdent(originalDatum)
