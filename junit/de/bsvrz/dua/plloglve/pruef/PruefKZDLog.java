...
implements ClientReceiverInterface {
	...
	/*
	 * TODO: Log Writer der aufrufenden Klasse
	 */
	...
	public PruefKZDLog(PlPruefungLogischLVETest caller, SystemObject[] fs, String csvQuelle)
	throws Exception {
	...
	private void csvEinlesen() {
		...
		try {
			...
			System.out.println("ERR CSV READ:"+e);
		}
	}
	...
	private void pruefungFertig() {
		if(pruefungFS2Fertig && pruefungFS3Fertig) {
			//TODO: Log
			System.out.println("Pruefung fertig:Benachrichtige Hauptthread");
			this.doWait();  //Warte 250ms
...
class VergleicheKZD extends Thread {
	 ...
	 */
	public VergleicheKZD(PruefKZDLog caller, Data daten, ArrayList<HashMap<String,Integer>> csvZeilen, int fsIndex, int csvOffset) {
	...
	private void pruefeDaten(ArrayList<HashMap<String,Integer>> csvZeilen, int fsIndex, int csvOffset) throws Exception {
		...
		System.out.println("PR:Pruefe Daten "+ident);
		...
		String attribut;
		caller.br.write(ident+"HashMap Groesse: CSV("+hmCSV.size()+") <> Results("+hmResult.size()+")\n\r");
		//System.out.println(ident+"HashMap Groesse: CSV("+hmCSV.size()+") <> Results("+hmResult.size()+")");
			...
			for(int j=0;j<attributNamen.length;j++) {
				...
				if(!attribut.equals("tNetto.Wert")) {
					if(!hmCSV.get(attribut).equals(hmResult.get(attribut))) {
						caller.br.write(ident+"DIF ("+attribut+"):"+ hmCSV.get(attribut) +" <> "+hmResult.get(attribut)+"\n\r");
						System.out.println(ident+"DIF ("+attribut+"):"+ hmCSV.get(attribut) +" <> "+hmResult.get(attribut));
					} else {
						caller.br.write(ident+"OK ("+attribut+"):"+ hmCSV.get(attribut) +" == "+hmResult.get(attribut)+"\n\r");
						//System.out.println(ident+"OK ("+attribut+"):"+ hmCSV.get(attribut) +" == "+hmResult.get(attribut));
					}
				} else {
					if(csvWerttNetto != resultWerttNetto) {
						caller.br.write(ident+"DIF ("+attribut+"):"+ csvWerttNetto +" <> "+resultWerttNetto+"\n\r");
						System.out.println(ident+"DIF ("+attribut+"):"+ csvWerttNetto +" <> "+resultWerttNetto);
					} else {
						caller.br.write(ident+"OK ("+attribut+"):"+ csvWerttNetto +" <> "+resultWerttNetto+"\n\r");
						//System.out.println(ident+"OK ("+attribut+"):"+ csvWerttNetto +" <> "+resultWerttNetto);
					}
				}
			}
		}
		...
		//TODO: Log
		caller.br.write("\n\r");
		System.out.println("HT:Benachrichtige Listener");
		caller.doNotify();  //Benachrichtige aufrufende Klasse
	}
