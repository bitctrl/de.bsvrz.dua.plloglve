...
extends AbstraktVerwaltungsAdapter{
	...
	throws DUAInitialisierungsException {				
		...
		Collection<SystemObject> plLogLveObjekte = DUAUtensilien.getBasisInstanzen(
				this.verbindung.getDataModel().getType("typ.fahrStreifen"),  //$NON-NLS-1$
		...
		DataDescription anmeldungsBeschreibungKZD = new DataDescription(
				this.verbindung.getDataModel().getAttributeGroup(PLLOGKonstanten.ATG_KZD),
				this.verbindung.getDataModel().getAspect("asp.externeErfassung"),  //$NON-NLS-1$
				(short)0);
		DataDescription anmeldungsBeschreibungLZD = new DataDescription(
				this.verbindung.getDataModel().getAttributeGroup(PLLOGKonstanten.ATG_LZD),
				this.verbindung.getDataModel().getAspect("asp.externeErfassung"),  //$NON-NLS-1$
				(short)0);
			
		Collection<DAVObjektAnmeldung> anmeldungen = new TreeSet<DAVObjektAnmeldung>();
		for(SystemObject obj:this.getSystemObjekte()){
			try{
				anmeldungen.add(new DAVObjektAnmeldung(obj, anmeldungsBeschreibungKZD));
				anmeldungen.add(new DAVObjektAnmeldung(obj, anmeldungsBeschreibungLZD));
			}catch(Exception ex){
				throw new DUAInitialisierungsException("Es konnten nicht alle Anmeldungen durchgeführt werden", ex); //$NON-NLS-1$
			}
		}
